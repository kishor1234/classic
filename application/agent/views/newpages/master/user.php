                                                ...
                                                <div class="row">
                                                    <div class="col-lg-12">
                                                        <form action="#" method="post" id="myMainForm">
                                                            ...
                                                            <input type="hidden" name="blockper" required id="blockper"  step="1"  min="1" max="100"  placeholder="Enter User Blocking Percentage" title="Enter User Blockging Percentage" autocomplete="off" class="form-control">
                                                                <input type="hidden" name="winper" required id="winper"  step="1"  min="10" max="100" value="10.00" placeholder="Enter User Wining Percentage" title="Enter User Wining Percentage" autocomplete="off" class="form-control">
                                                            <!--                                                                <span id="error_name" class=""></span>
                                                                                                                        </div>-->
<!--                                                            <div class="form-group">
                                                                <label class="form-control-label">Winning sender Percentage(1% to 100%) </label>
                                                                <input type="number" name="blockper" required id="blockper"  step="1"  min="1" max="100"  placeholder="Enter User Blocking Percentage" title="Enter User Blockging Percentage" autocomplete="off" class="form-control">
                                                                <span id="error_name" class=""></span>
                                                            </div>-->
                                                            <div class="form-group">
                                                                <input type="hidden" name="action" id="action" value="addUser">
                                                                <input type="hidden" name="agent_id" id="agent_id" value="<?= $_SESSION["userid"] ?>">
                                                                <input type="hidden" name="id" id="id" value="0">
                                                                <input type="hidden" name="dist_id" value="<?= $_SESSION["createBy"] ?>">
                                                                <!--<input type="hidden" name="agent_id" value="<?= $_SESSION["userid"] ?>">-->
                                                    ...
                                                    <div class="col-lg-12">
                                                        <form action="#" method="post" id="myPointForm">
                                                            ...
                                                            <div class="form-group">
                                                                ...
                                                                <input type="text" name="agent_id" value="<?= $_SESSION["userid"] ?>">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#myTable').DataTable({
            ...
            ajax: {
                url: '<?= api_url ?>/?r=CAddAgentUser',
                type: "post", // method  , by default get
                dataType: "json",
                data: {action: "loadTable", id: "<?= $_SESSION["userid"] ?>"},
//                success: function (d) {
//                    console.log(d);
//                },
    ...
    function deleteFullAccount(id, st)
    {
        swal({
            ...
            closeOnConfirm: true
                ...
                function () {
                    $.post('<?= api_url ?>/?r=CAddAgentUser', {agent_id: "<?= $_SESSION["userid"] ?>", id: id, action: 'delete'}, function (data) {
                        console.log(data);
                        table.ajax.reload(null, false);
                        var json = JSON.parse(data);
                        $.toaster({priority: json.toast[0], title: json.toast[1], message: json.toast[2]});
    ...
    function editAccount(id, name, mobile, commision, device, winper, block)
    {
        $("#myMain").modal("show");
        $("#name").val(name);
        $("#mobileno").val(mobile);
    ...
    function putPoint(id, name, point)
    {
        swal({
            ...
            {
                $.post('<?= api_url ?>/?r=CAddAgentUser', {agent_id: "<?= $_SESSION["userid"] ?>", userid: id, point: inputValue, action: 'putPoint'}, function (data) {
                    console.log(data);
                    var json = JSON.parse(data);
                    if (json.status === 1)
                    {
    ...
    function getPoint(id, name, point)
    {
        swal({
            ...
            if ($.isNumeric(inputValue))
            {
                $.post('<?= api_url ?>/?r=CAddAgentUser', {agent_id: "<?= $_SESSION["userid"] ?>", userid: id, point: inputValue, action: 'getPoint'}, function (data) {
                    console.log(data);
                    var json = JSON.parse(data);
                    if (json.status === 1)
                    {
    ...
    function suspendAccount(id, status)
    {
        ...
        swal({
            ...
            closeOnConfirm: true
                ...
                function () {
                    $.post('<?= api_url ?>/?r=CAddAgentUser', {id: id, is_active: status, action: 'suspend'}, function (data) {
                        console.log(data);
                        table.ajax.reload(null, false);
                        var json = JSON.parse(data);
                        $.toaster({priority: json.toast[0], title: json.toast[1], message: json.toast[2]});
